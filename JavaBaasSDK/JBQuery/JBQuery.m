...
- (void)findObjectsInBackgroundWithBlock:(JBArrayResultBlock)block {
    ...
    [HttpRequestManager queryObjectWithUrlPath:urlPath queryParam:param success:^(NSURLSessionDataTask *task, id responseObject) {
        BOOL temp = NO;
...
- (void)getObjectInBackgroundWithId:(NSString *)objectId block:(JBObjectResultBlock)block {
    ...
    [HttpRequestManager getObjectWithUrlPath:urlPath parameters:param success:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)deleteAllInBackgroundWithBlock:(JBBooleanResultBlock)block {
    ...
    [HttpRequestManager deleteObjectWithUrlPath:urlPath queryParam:param success:^(NSURLSessionDataTask *task, id responseObject) {
        block(YES, nil);
    } failure:^(JBResponse *jbResponse, NSError *error) {
        if (jbResponse) {
            NSError *customError = [NSError errorWithDomain:jbResponse.message code:jbResponse.code userInfo:@{NSLocalizedDescriptionKey: jbResponse.message}];
